...
def onmi(cover,coverRef,allNodes=None,variant=None,ignore=True): #cover and coverRef should be list of set, no community ID
    """
    ...
    eps = 1e-45
    if variant is None:
        NMI = 1- 0.5*(HXY/(HX+eps)+HYX/(HY+eps))
        IXY = 0.5*(HX-HXY+HY-HYX)
        NMI_MGH =  IXY/(max(HX,HY) + eps)
        return NMI,NMI_MGH
